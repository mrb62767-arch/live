...
# The actual syntax in entrypoint.sh is: ${RECORDING_SCREEN_WIDTH:-'1920'}
# so we replace the fallback default values, not a plain assignment.
RUN chmod +x /*.sh 2>/dev/null; chmod +x ./*.sh 2>/dev/null; \
    find / -maxdepth 3 -name "*.sh" -exec chmod 755 {} \; 2>/dev/null; \
    sed -i "s/:-'1920'/:-'960'/g" ./entrypoint.sh 2>/dev/null; \
    sed -i "s/:-'1080'/:-'540'/g" ./entrypoint.sh 2>/dev/null; \
    ...
    echo "permissions and resolution fixed, cachebust=4"
